...
There are `r observations` observations. Clean data by removing NAs (not measured steps during interval):
...
Now we have `r observations_clean` observations.  
...
Mean value is `r mdrm_mean ` and median value is `r mdrm_median`.
...
Maximum average of steps was `r max_steps` and it was in interval: `r max_interval`.
...
There is `r missing` values where steps were not measured.
...
Total count of new dataset called md_imputs  is `r mdfc`. Now once again we print histogram from new dataset:
...
Mean value is `r mdrm_mean2 ` and median value is `r mdrm_median2`. Impact of imputed data is: new median is smaller and mean is as before.
